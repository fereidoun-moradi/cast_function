...
The shift equivalence relation exists between states and the values of the time shifting are tagged on transitions. For example, the transition from state $S\_9$ to state $S\_8$ has the time shifting with a non-zero value. The label $(20\gg 10)$ indicates that the time value $20$ is shifted to the time value $10$ by the amount of $10$.
